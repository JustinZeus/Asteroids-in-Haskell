...
step secs gstate
  ...
  |collisionWithAsteroid (player gstate) (asteroids gstate) == False = return $ gstate {player = updatePlayer (player gstate), bullets = moveBullet (bullets gstate), elapsedTime = elapsedTime gstate + secs}
...
inputKey (EventKey (Char c) _ _ _) gstate
   -- if any key other than z show player
    ...
    | c == 'm' = gstate {asteroids = spawnAsteroid:spawnAsteroid2:spawnAsteroid3:spawnAsteroid4:[]}
...
spawnAsteroid3 = Asteroid (Collider (-200,300) 30) (0,0)

spawnAsteroid4 :: Asteroid
spawnAsteroid4 = Asteroid (Collider (200,300) 30) (0,0)
